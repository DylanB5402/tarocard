...
describe('Testing Drinks Database', function () {
  ...
  it('Test: Add New Drink, Correct Establishment', function () {
    const establishment_id = establishmentsDb.addEstablishment('mcdonalds')
    const id = drinksDb.addDrink('drink1', undefined, establishment_id, undefined)

    assert.equal(drinksDb.getDrink(id).establishment_id, establishment_id)
  })
  ...
  it('Test: Edit Drink, Correct Establishment', function () {
    ...
    const desc = 'a sour drink for summer'
    const id = drinksDb.addDrink(name, desc)
    const establishment_id = establishmentsDb.addEstablishment('china')

    drinksDb.editDrink(id, undefined, undefined, establishment_id)
